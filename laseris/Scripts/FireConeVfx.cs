...
namespace Game.Combat
{
	public partial class FireConeVfx : Node2D
	{
		...
		[ExportGroup("Extend / Occlusion (Strong Fire)")]
		[Export] public bool ExtendPastTargetToScreen = true;
		[Export] public float ScreenExtendMultiplier = 1.25f;

		[ExportGroup("Draw Order")]
		[Export] public bool ForceZLayers = true;
		[Export] public int ConeZIndex = -20;
		...
		// Shader fallback embutido (mesmo do .gdshader acima)
...
";
		...
		public void Play(Vector2 from, Vector2 to, float travelSec = 0.06f)
		{
			...
			// distância real até o alvo: define a "abertura"
			float lenToTarget = (to - from).Length();
			if (lenToTarget < 2f) lenToTarget = 2f;

			// ponto estendido pra atravessar tela
			Vector2 farTo = to;
			if (ExtendPastTargetToScreen)
			{
				Vector2 dir = (to - from).Normalized();
				float viewDiagWorld = GetViewDiagonalWorld();
				farTo = to + dir * (viewDiagWorld * ScreenExtendMultiplier);
			}

			AlignBetween(from, farTo, lenToTarget);

			// partículas ficam no inimigo (impacto)
		...
		private void EnsureInit()
		{
			...
			// ✅ Se você setou um ShaderMaterial no Mesh pelo Inspector, usamos ELE (duplica).
			//    Se não setou, aplica o fallback embutido (ShaderCode).
			var baseMat = _mesh.Material as ShaderMaterial;
			if (baseMat != null && baseMat.Shader != null)
				_mat = (ShaderMaterial)baseMat.Duplicate(true);
			else
		...
		private void AlignBetween(Vector2 from, Vector2 farTo, float widthLen)
		{
			Vector2 dir = farTo - from;
			float len = Mathf.Max(2f, dir.Length());
			Vector2 n = dir / len;

			GlobalPosition = (from + farTo) * 0.5f;
			GlobalRotation = n.Angle();

			// ✅ largura baseada no alvo (widthLen), não no farTo
			float flameWidthPx = EndWidthPxBase + (widthLen / 100f) * EndWidthPxPer100px;
			...
			float endRel   = Mathf.Clamp(flameWidthPx / meshHeightPx, 0.05f, 0.96f);
		...
		private float GetViewDiagonalWorld()
		{
			var vp = GetViewport();
			if (vp == null) return 2000f;

			Rect2 rect = vp.GetVisibleRect();

			// canvas->screen é GetCanvasTransform, então screen->canvas é o inverso
			Transform2D inv = vp.GetCanvasTransform().AffineInverse();

			Vector2 w0 = inv * rect.Position;
			Vector2 w1 = inv * (rect.Position + rect.Size);

			return Mathf.Max(200f, w0.DistanceTo(w1));
		}

		private void ApplyZLayers()
		{
			if (!ForceZLayers) return;

			if (_mesh != null)
			{
				_mesh.ZAsRelative = false;
				_mesh.ZIndex = ConeZIndex;
			}

			if (_endFlames != null)
			{
				_endFlames.ZAsRelative = false;
				_endFlames.ZIndex = ImpactZIndex;
			}

			if (_endEmbers != null)
			{
				_endEmbers.ZAsRelative = false;
				_endEmbers.ZIndex = ImpactZIndex;
			}
		}
